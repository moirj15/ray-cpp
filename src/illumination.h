...
class IlluminationModel
{
  ...
  public:
    ...
    IlluminationModel(const Scene &scene, f32 ambient_coef, f32 specular_coef, f32 diffuse_coef, f32 specular_exp, f32 reflection_const,
        f32 refraction_const) : _scene(scene),
    ...
    [[nodiscard]] virtual glm::vec4 illuminate(const IntersectData &id) const = 0;
};
...
class Phong : public IlluminationModel
{
  ...
  public:
    Phong(const Scene &scene,f32 a_coef, f32 s_coef, f32 d_coef, f32 s_exp, f32 r_const, f32 t_const, glm::vec4 aM, glm::vec4 dM,
        glm::vec4 sM) :
            IlluminationModel(scene,a_coef, s_coef, d_coef, s_exp, r_const, t_const),
            ambient_mat(aM), diffuse_mat(dM), specular_mat(sM)
    {
    }

    [[nodiscard]] glm::vec4 illuminate(const IntersectData &id) const override;
};
...
class CheckerBoard : public IlluminationModel
{
  ...
  public:
    CheckerBoard(const Scene &scene,
