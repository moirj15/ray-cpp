 ...
 */
static inline void fill_triangle(glm::vec4 triangle[3], Canvas *can, u32 value)
{
...
void draw_vbo(VBO vbo, Canvas *can, std::vector<u32> values)
{
    ...
    for (u64 i = 0; i < vbo.connections.size(); i += 3) {
        ...
        glm::vec4 triangle[3] = {vbo.vertexList[vbo.connections[i]], vbo.vertexList[vbo.connections[i + 1]], vbo.vertexList[vbo.connections[i + 2]]};
